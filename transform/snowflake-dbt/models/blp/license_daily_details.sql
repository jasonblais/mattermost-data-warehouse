...
WITH license_daily_details_all as (
     ...
     license_daily_details AS (
        SELECT
          ...
          , MAX(ld.master_account_sfid) OVER (PARTITION BY ld.customer_id)                         AS master_account_sfid
          , MAX(ld.master_account_name) OVER (PARTITION BY ld.customer_id)                         AS master_account_name
          , MAX(ld.account_sfid) OVER (PARTITION BY ld.customer_id)                                AS account_sfid
          , MAX(ld.account_name) OVER (PARTITION BY ld.customer_id)                                AS account_name
          , ld.license_email
          , ld.contact_sfid
          , ld.contact_email
          , ld.number
